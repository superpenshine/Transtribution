...
class GradeQuerySet(models.QuerySet):
    ...
    # Grade count, avg, max, min, passed grade number of all test (test is differentiated by test+subject+className)
    # example: class_grades_stats = Grade.objects.get_grade_stats(pass_grade=60)
    def get_grade_stats(self, pass_grade=60):
        return self.values('test', 'subject', 'name_id__class_name').annotate(
            count=Count('id'), 
            avg=Avg('score'), 
            max=Max('score'), 
            min=Min('score'), 
            pass_num=Count('id', filter=Q(score__gte=pass_grade))
        )

    # Grade count, avg, max, min, passed grade number of specified test
    def get_test_stats(self, subject, test, pass_grade=60):
        return self.filter(subject=subject, test=test).aggregate(
            count=Count('id'), 
            avg=Avg('score'), 
            max=Max('score'), 
            min=Min('score'), 
